...
function confirmVendorCancel() {
  confirmGeneralCancel('my_vendors');
}

function confirmProductCancel() {
  confirmGeneralCancel('my_products');
}
...
function addNewSelect(tagName, tagId) {
  ...
  let formFor = $(document)
    .find('form')
    .attr('class')
    .split('_')[1]; // Value is vendor or product
  ...
  $('<input>')
    ...
    .appendTo(newDiv);
  $('<input>')
    .attr({ name: namePrefix + '[_destroy]', type: 'hidden', value: 0 })
    .appendTo(newDiv);
  $('<input>')
    .attr({
      type: 'checkbox',
      ...
      id: idPrefix + '_destroy'
    })
    .appendTo(newDiv);
  $('<label>')
    .attr({ for: idPrefix + '_destroy' })
    .text('Remove Tag')
    .appendTo(newDiv);
  ...
  if (tagId) {
    ...
    $('<option>')
      .attr({ value: tagId, selected: 'selected' })
      .appendTo($(`#${formFor}_${tagType}_ids`));
  }
}
